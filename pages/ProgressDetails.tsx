...
const ProgressDetails = () => {
  ...
  const handleBegin = task => {
    const updatedTasks = tasks.map(t =>
      t.name === task.name
        ? {...t, status: new Date().toLocaleTimeString()}
        : t,
    );
  ...
  const handleComplete = task => {
    const updatedTasks = tasks.map(t =>
      t.name === task.name
        ? {...t, progress: 100, status: new Date().toLocaleTimeString()}
        : t,
    );
    ...
    <View style={styles.container}>
      ...
      <View style={styles.buttonContainer}>
        ...
        <View style={styles.button}>
          <Button
            title="In Progress"
            onPress={() => handleInProgress(selectedTask)}
          />
        </View>
        <View style={styles.button}>
          <Button
      ...
      {tasks.map((task, index) => (
        <TouchableOpacity
          ...
          style={[
            ...
            {
              backgroundColor:
                task.status !== 'Waiting' ? '#F5F5F5' : 'lightgray',
            },
          ]}>
          <View>
